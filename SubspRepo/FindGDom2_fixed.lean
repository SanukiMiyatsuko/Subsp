...
#eval match bad with | none=>"none" | some s=>"src -> " ++ reprStr (_root_.trans s)++" oldG="++toString ((T.G1 0 (_root_.trans s)).map reprStr)++" newGtrans="++toString ((new.T.G s).map (fun y=>reprStr (_root_.trans y)))++" bad="++match badX s with | none=>"?" | some x=>reprStr x
