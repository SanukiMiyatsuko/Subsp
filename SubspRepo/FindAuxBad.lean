...
def bad := vecs.find? (fun v => match badx (auxSum v) with | none=>false | some _=>true)
#eval match bad with | none=>"none" | some v => let s:=auxSum v; reprStr s ++ " G=" ++ toString ((T.G1 0 s).map reprStr) ++ " bad=" ++ match badx s with | none=>"?" | some x=>reprStr x ++ " cmp=" ++ toString (T.decLt x s) ++ " rev=" ++toString (T.decLt s x)
