...
def prop (a:new.T 2):Bool := if a == new.T.Z || !(og (trans a)) then true else let E:=T.early_collapse (trans a); let C:=T.card_times 1 (T.one_del E); decide (trans a < T.P 1 C Z)
def xs:=gen2 3
#eval xs.all prop
