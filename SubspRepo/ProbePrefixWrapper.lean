...
def prop (v:new.Vec (new.T 2) 2):Bool := let p:=pref v; (T.G1 0 p).all (fun x => decide (x < T.P 1 p T.Z))
def bad := vecs.find? (fun v => (transAux v).1 && !(prop v))
#eval match bad with | none=>"none" | some v => let p:=pref v; reprStr p ++ " bad=" ++ match (T.G1 0 p).find? (fun x => !(decide (x < T.P 1 p T.Z))) with | none=>"?" | some x=>reprStr x
