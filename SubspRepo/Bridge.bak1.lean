...
theorem bridge_card_times_head (n : Nat) (s : T) :
  ...
  cases s with
  | Z =>
    rw [T.card_times.eq_1, T.head, T.card_times.eq_1]
  | P p a b =>
    cases n with
    | zero =>
      rw [T.card_times.eq_2, T.head, T.card_times.eq_2]
    | succ k =>
      rw [T.card_times.eq_3, T.head, T.card_times.eq_3, T.card_times.eq_1]
      by_cases hp : p = 0
      · rw [ite_eq_left hp, ite_eq_left hp, T.P_add_eq, T.add.eq_1, T.add_Z]
      · rw [ite_eq_right hp, ite_eq_right hp, T.P_add_eq, T.add.eq_1, T.add_Z]

#print axioms bridge_card_times_head
        ...
        · constructor
          ...
          · intro x hx
            ...
            | inl hleft =>
              cases hleft with
              | inl hmid =>
                have hxmid := bridge_shift_strong1 k (T.early_collapse a) hec.2.1 x
                  (List.mem_singleton.mp hmid ▸
                ...
                rw [hmidEq]
                exact T.Lt.p_mid 1 _ _ _ _
                  (bridge_shift_lt_wrap k (T.early_collapse a) hec.2.1)
              | inr hG =>
                have hxmid := bridge_shift_strong1 k (T.early_collapse a) hec.2.1 x hG
                exact T.Lt.p_mid 1 x
                  (T.add (T.mul (T.P 1 T.Z T.Z) (T.ofNat k))
                    (T.early_collapse a))
                  x (T.card_times (k + 1) b) hxmid
            | inr htailG =>
              have hxtail := htail.2.2 x htailG
              exact lt_trans_thm x (T.card_times (k + 1) b)
                (T.P 1
                  (T.add (T.mul (T.P 1 T.Z T.Z) (T.ofNat k))
                    (T.early_collapse a))
                  (T.card_times (k + 1) b))
                ...
                (T.isNF1_tail_le _ hcurNF 1
                  (T.add (T.mul (T.P 1 T.Z T.Z) (T.ofNat k))
                    (T.early_collapse a))
                  (T.card_times (k + 1) b) rfl |>.resolve_right (by
                    intro heq
                    have hne := bridge_card_times_ne_Z (k + 1) b
                    by_cases hbZ : b = T.Z
                    · rw [hbZ] at heq
                      change T.Z = T.P 1 _ T.Z at heq
                      cases heq
                    · exact hne hbZ (by
                        rw [heq]
                        intro hh
