...
theorem bridge_stand_P_NF1 (n : Nat) (a b : T)
  ...
  by_cases_dec hhead : T.head b ≤ T.P n a T.Z
...
theorem bridge_stand_ne_Z (s : T) (hs : s ≠ T.Z) :
  ...
  | P p a b iha ihb =>
    rw [T.stand]
    by_cases_dec hle : T.head (T.stand b) ≤ T.P p a T.Z
    · rw [ite_eq_left hle]
      intro h
      cases h
    · rw [ite_eq_right hle]
