...
theorem T.SDom_PZ_pivot {lam : Nat}
  ...
  · exact T.P_lt_P_of_compareVec_lt low high T.Z T.Z
      (Vec.compare_lt_of_pivot low high i hAbove hPivot.1)
  · intro c hlc hch
    cases c with
    | Z =>
      cases hlc with
    ...
    | P mid add =>
      ...
      have hBetween :=
        Vec.between_pivot low mid high i hAbove hPivot.1
          hLowVec hHighVec
      intro x hx
        ...
        | inr hrest =>
          ...
          | inl hiq =>
            have hLowMid : low.idx i ≤ mid.idx i :=
              hBetween.2.1
            have hMidHigh : mid.idx i ≤ high.idx i :=
              hBetween.2.2
            ...
            | inl hxuEq =>
              ...
              rw [hxLow]
              refine ⟨mid.idx i, ?_, hLowMid⟩
              apply List.mem_append_left (T.GZ z)
              apply (T.mem_G_P mid add (mid.idx i)).mpr
              have hmem : mid.idx i ∈ Vec.toList mid :=
                (Vec.mem_toList_iff_idx mid (mid.idx i)).mpr ⟨i, rfl⟩
              exact Or.inl ⟨mid.idx i, hmem, Or.inl rfl⟩
            | inr hG =>
              rw [huEq] at hG
              obtain ⟨y, hy, hxy⟩ :=
                hPivot.2 (mid.idx i) hLowMid hMidHigh x hG
              cases List.mem_append.mp hy with
              | inl hyG =>
                refine ⟨y, ?_, hxy⟩
                apply List.mem_append_left (T.GZ z)
                apply (T.mem_G_P mid add y).mpr
                have hmem : mid.idx i ∈ Vec.toList mid :=
                  (Vec.mem_toList_iff_idx mid (mid.idx i)).mpr ⟨i, rfl⟩
                exact Or.inl ⟨mid.idx i, hmem, Or.inr hyG⟩
              | inr hyZ =>
                exact ⟨y, List.mem_append_right (T.G (T.P mid add)) hyZ, hxy⟩
          | inr hiq =>
            ...
            have hmem : mid.idx q ∈ Vec.toList mid :=
              (Vec.mem_toList_iff_idx mid (mid.idx q)).mpr ⟨q, rfl⟩
            cases hxu with
            | inl hxuEq =>
              exact Or.inl ⟨mid.idx q, hmem, Or.inl (hxuEq.trans huMid)⟩
            | inr hG =>
              rw [huMid] at hG
...
theorem T.fund_Omega_strict_mono {lam : Nat} (s : T lam)
    (hd : T.dom s = .Omega) :
    ∀ x y : T lam, x < y → T.fund s x < T.fund s y := by
  induction s with
  | Z =>
    cases hd
  | P ls add ih =>
          ...
          | one =>
            ...
            · rw [if_neg hm0] at hd
              ...
              | mk mv mh =>
                ...
                | succ k =>
                  let mi : Fin lam := ⟨k + 1, mh⟩
                  let mj : Fin lam :=
                    ⟨k, Nat.lt_of_succ_lt mh⟩
                  let base :=
                    ls.rplc mi (T.fund ls[mi] T.Z)
                  rw [T.fund, if_pos rfl, hmin,
                    if_pos rfl, if_neg (Nat.succ_ne_zero k)]
                  have hcmp :
                      compareVec (base.rplc mj x)
                        (base.rplc mj y) = Ordering.lt := by
                    apply Vec.compare_lt_of_pivot
                      (base.rplc mj x) (base.rplc mj y) mj
                    · intro q hjq
                      have hqj : q.val ≠ mj.val :=
                        Nat.ne_of_gt hjq
                      rw [Vec.rplc_idx_of_ne base mj q x hqj]
                      rw [Vec.rplc_idx_of_ne base mj q y hqj]
                    · rw [Vec.rplc_idx_same]
                      rw [Vec.rplc_idx_same]
                      exact hxy
                  exact T.P_lt_P_of_compareVec_lt
                    (base.rplc mj x) (base.rplc mj y)
                    T.Z T.Z hcmp
    · have hdadd : T.dom add = .Omega := by
        rw [T.dom] at hd
...
theorem T.iter_fund_lt_fund_Omega {lam : Nat}
  ...
  | Z =>
    rw [T.iter]
    have hne := T.fund_Omega_ne_Z s T.Z hd
    cases T.Z_le (T.fund s T.Z) with
    | inl hlt => exact hlt
    | inr heq => exact False.elim (hne heq.symm)
  | P ls add ih =>
    rw [T.iter]
    exact T.fund_Omega_strict_mono s hd
      (T.iter (fun x => T.fund s x) add)
      (T.fund s (T.iter (fun x => T.fund s x) add))
      ih
